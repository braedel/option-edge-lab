...
def run(train_months, val_months):
    ...
    for name, model in [("Ridge", Ridge(alpha=1.0)),
        ...
        model.fit(Xtr, ytr)
        ic_is = float(stats.spearmanr(model.predict(Xtr), ytr)[0])
        pos = model.predict(Xv)
        ic_oos = float(stats.spearmanr(pos, yv)[0])
        net = np.sign(pos) * yv - COST            # trade the predicted direction, pay spread+fee
        thr = np.percentile(np.abs(pos), 90)
        conf = np.abs(pos) >= thr
        net_conf = float((np.sign(pos[conf]) * yv[conf] - COST).mean())
        results[name] = {"ic_in": ic_is, "ic_oos": ic_oos, "taker_net_oos": float(net.mean()),
                         "win": float(np.mean(net > 0)), "top_decile_conf_net": net_conf}
        print(f"  {name:5s} IC_in={ic_is:+.4f}  IC_oos={ic_oos:+.4f}  "
              f"taker_net_oos={net.mean():+.3f}t (win {np.mean(net > 0):.3f})  "
              f"top-decile-conf net={net_conf:+.3f}t n={conf.sum()}")
    out = Path("reports/zb_taker")
    out.mkdir(parents=True, exist_ok=True)
    rec = {"train_months": list(train_months), "val_months": list(val_months),
           "n_train": int(len(ytr)), "n_oos": int(len(yv)), "results": results,
           "note": "OOS multivariate signal check; 13 causal features; GBR frozen on TRAIN; "
                   "val months are forward OOS. UNCONDITIONAL IC -- a maker edge requires the "
                   "conditional-on-fill test (c25), not this."}
    (out / "multivariate.json").write_text(json.dumps(rec, indent=2))
    print("  -> wrote reports/zb_taker/multivariate.json")


if __name__ == "__main__":
    run(["2024-05", "2024-06", "2024-07"], ["2025-06", "2025-07", "2025-08"])  # 3 forward-OOS months, one committed run
